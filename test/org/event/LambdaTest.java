// @Hogler
// - http://stackoverflow.com/users/2711488/holger
// - http://stackoverflow.com/questions/19557829/faster-alternatives-to-javas-reflection
package org.event;

import java.lang.invoke.CallSite;
import java.lang.invoke.LambdaMetafactory;
import java.lang.invoke.MethodHandle;
import java.lang.invoke.MethodHandles;
import java.lang.invoke.MethodType;
import java.lang.reflect.Method;
import java.util.function.IntBinaryOperator;

public class LambdaTest {

    private static final LambdaTest obj = new LambdaTest();
    private static final int ITERATIONS = 50_000_000;
    private static final int WARM_UP = 10;
    
    public static void mains(String... args) throws Throwable {
        // hold result to prevent too much optimizations
        final int[] dummy = new int[4];

        final MethodHandles.Lookup caller = MethodHandles.lookup();
        Method reflected = LambdaTest.class.getDeclaredMethod("myMethod", int.class, int.class);
        MethodHandle mh = caller.unreflect(reflected);
        MethodType getter = MethodType.methodType(int.class, int.class, int.class);
        MethodHandle target = caller.findVirtual(LambdaTest.class, "myMethod", getter);
        CallSite site = LambdaMetafactory.metafactory(
                caller, "applyAsInt", MethodType.methodType(IntBinaryOperator.class, LambdaTest.class),
                getter, target, getter);
        MethodHandle factory = site.getTarget();
        factory = factory.bindTo(obj);
        IntBinaryOperator lambda = (IntBinaryOperator)factory.invoke();
        
        for (int i = 0; i < WARM_UP; i++) {
            dummy[0] += testDirect(dummy[0]);
            dummy[1] += testLambda(dummy[1], lambda);
            dummy[2] += testMH(dummy[1], mh);
            dummy[3] += testReflection(dummy[2], reflected);
        }
        long t0 = System.nanoTime();
        dummy[0] += testDirect(dummy[0]);
        long t1 = System.nanoTime();
        dummy[1] += testLambda(dummy[1], lambda);
        long t2 = System.nanoTime();
        dummy[2] += testMH(dummy[1], mh);
        long t3 = System.nanoTime();
        dummy[3] += testReflection(dummy[2], reflected);
        long t4 = System.nanoTime();
        System.out.printf("direct: %.2fs, lambda: %.2fs, mh: %.2fs, reflection: %.2fs%n",
                (t1 - t0) * 1e-9, (t2 - t1) * 1e-9, (t3 - t2) * 1e-9, (t4 - t3) * 1e-9);

        // do something with the results
        if (dummy[0] != dummy[1] || dummy[0] != dummy[2] || dummy[0] != dummy[3]) {
            throw new AssertionError();
        }
    }

    private static int testMH(int v, MethodHandle mh) throws Throwable {
        for (int i = 0; i < ITERATIONS; i++) {
            v += (int) mh.invokeExact(obj, 1000, v);
        }
        return v;
    }

    private static int testReflection(int v, Method mh) throws Throwable {
        for (int i = 0; i < ITERATIONS; i++) {
            v += (int) mh.invoke(obj, 1000, v);
        }
        return v;
    }

    private static int testDirect(int v) {
        for (int i = 0; i < ITERATIONS; i++) {
            v += obj.myMethod(1000, v);
        }
        return v;
    }

    private static int testLambda(int v, IntBinaryOperator accessor) {
        for (int i = 0; i < ITERATIONS; i++) {
            v += accessor.applyAsInt(1000, v);
        }
        return v;
    }

    private int myMethod(int a, int b) {
        return a < b ? a : b;
    }
}
